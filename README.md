...
The current version covers the transportation network from 1994 to 2024.
...
This data set records the prefecture-to-prefecture travel time in hours for three modes of transportation: road, railroad (freight), and railroad (passenger). The current version covers the years 1994 to 2024 and contains 279 prefectures.
...
The `cityinfo.csv` file contains the basic information about the 279 prefectures.
...
The following files contain the travel time matrix:
...
The travel time data files share the same structure. Each file contains $38781$ rows, corresponding to the lower triangle of the $279\times279$ symmetric travel time matrix (excluding the diagonal). The variables in these files are:
1. The first two columns, `origin` and `destination`, are the four-digit administrative division codes of the origin and destination prefectures.
...
1. `seg_id`: the unique index of a segment that the pixel belongs to. The `seg_id` matches the identifier used in the segment-level dataset, with details [here](https://github.com/malin84/transportation_networks_of_china?tab=readme-ov-file#segment-level-information-name-year-of-construction-and-applicable-design-code).
...
8. `time`: the time required to traverse the pixel in hours. See the note below on its computation.
...
Notes:
1. To compute `time` from `speed`, we use the following equation: $time = distance / speed$. The variable $distance$ is the edge length to a cardinal-adjacent pixel (0.5097 km at this resolution).
...
All the files are stored in the folder [seg_info](seg_info/). Each mode has three data files:
